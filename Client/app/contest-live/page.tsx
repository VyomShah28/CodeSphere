...
import { ArrowLeft, Clock, Send, CheckCircle, XCircle, Sun, Moon, Settings, Grid3X3 } from "lucide-react"
...
export default function ContestLive() {
  ...
  const [currentProblem, setCurrentProblem] = useState<Challenge>({} as Challenge)
...
console.log(solution());`,
  ...
  const fetchChallenge = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/get-challenge-by-id?challengeId=${challengeId}`)
  ...
  return (
    <div className={`h-screen flex flex-col overflow-hidden ${isDarkMode ? "dark bg-slate-900" : "bg-slate-50"}`}>
      {/* Header */}
      <header className={`border-b flex-shrink-0 ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white"}`}>
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="icon" onClick={() => setShowExitModal(true)}>
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => router.push("/contest-waiting?contestId=" + contestId)}>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Badge
              variant="secondary"
              className={`${
                isDarkMode ? "bg-slate-700 text-slate-200 border-slate-600" : "bg-slate-100 text-slate-800"
              }`}
            >
              Live Contest
            </Badge>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        {/* Problem Statement */}
        <ResizablePane direction="horizontal" initialSize={45} minSize={25} maxSize={65} className="flex flex-col">
          <div
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center space-x-2">
                  <Badge className={getDifficultyColor(currentProblem.difficulty_level)}>
              ...
              <Tabs defaultValue="problem" className="w-full">
                ...
                <TabsContent value="problem" className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className={isDarkMode ? "text-slate-300" : "text-slate-700"}>{currentProblem.input_formate}</p>
                  </div>

                  <div>
                    <h3 className={`font-semibold mb-2 ${isDarkMode ? "text-white" : ""}`}>Output Format</h3>
                    <p className={isDarkMode ? "text-slate-300" : "text-slate-700"}>{currentProblem.output_form}</p>
                  </div>
                </TabsContent>
                  ...
                  <div>
                    ...
                    <pre
                      className={`p-3 rounded text-sm font-mono ${
                        isDarkMode ? "bg-slate-700 text-slate-200" : "bg-slate-100"
                      }`}
                    >
        ...
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            <ResizablePane direction="vertical" initialSize={60} minSize={30} maxSize={80} className="flex flex-col">
              <div className={`flex-1 relative overflow-hidden ${isDarkMode ? "bg-slate-900" : "bg-white"}`}>
                <CodeEditor
                  value={code}
            ...
            {/* Terminal */}
            <div
              className={`flex flex-col flex-1 min-h-0 overflow-hidden ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"} border-t`}
            >
      ...
      {submissionResult && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className={`p-6 rounded-lg max-w-md w-full mx-4 ${isDarkMode ? "bg-slate-800 text-white" : "bg-white"}`}>
            <div className="text-center">
              <div
                className={`mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-4 ${
                  submissionResult.status === "accepted" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"
                }`}
              >
              ...
              <h3 className="text-lg font-semibold mb-2">
                {submissionResult.status === "accepted" ? "Solution Accepted!" : "Solution Needs Improvement"}
              </h3>
              ...
              <p className="text-xs mt-4 text-slate-500">Redirecting to contest page...</p>
            </div>
          </div>
        </div>
      )}
