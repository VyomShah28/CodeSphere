...
export function TestcaseRunner({
  ...
  const runAllTests = async () => {
    ...
    for (let i = 0; i < resetTests.length; i++) {
      ...
      setTestCases((prev) => prev.map((tc) => (tc.id === i + 1 ? { ...tc, status: "running" as const } : tc)))

      try {
        const response = await fetch("http://localhost:8000/api/runcode", {
          method: "POST",
          headers: {
  ...
  const getStatusBadge = (status: TestCase["status"]) => {
    switch (status) {
      ...
      case "error":
        return (
          <Badge variant="secondary" className="text-orange-600 bg-orange-100 text-xs">
            ERROR
          </Badge>
        )
      default:
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          <Loader2
            className={`h-8 w-8 mx-auto mb-3 animate-spin ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}
          />
          <p className={`text-sm ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>Loading test cases...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-4">
          ...
          {passedCount > 0 && passedCount === totalCount && (
            <Badge variant="secondary" className="text-green-600 bg-green-100 text-sm">
              All Passed ✓
            </Badge>
          )}
        </div>
      ...
      {isRunning && (
        <div className="mb-4">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Progress: {Math.round(progress)}% ({Math.ceil(progress / (100 / totalCount))}/{totalCount} tests completed)
          </p>
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto space-y-3">
        {testCases.length > 0 ? (
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {(testCase.time || testCase.memory) && (
                  <div className="flex items-center space-x-4 text-xs">
                    {testCase.time && (
                      <span className={`${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>{testCase.time}</span>
                    )}
                    {testCase.memory && (
                      <span className={`${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>{testCase.memory}</span>
                    )}
                  </div>
                )}
              </div>
                ...
                <div className={`${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>
                  ...
                  <code
                    className={`px-2 py-1 rounded text-xs font-mono ${
                      isDarkMode ? "bg-slate-600 text-slate-200" : "bg-slate-200 text-slate-800"
                    }`}
                  >
                    {testCase.input}
                  </code>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <AlertCircle className={`h-12 w-12 mx-auto mb-3 ${isDarkMode ? "text-slate-600" : "text-slate-400"}`} />
              <p className={`text-sm ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>No test cases available</p>
